...
fun! vm#maps#all#permanent() abort
  ...
  " map <c-n> in any case
  let maps["Find Under"][0]              = '<A-d>'
  let maps["Find Subword Under"][0]      = '<A-d>'

  if g:VM_default_mappings
    "let maps["Reselect Last"][0]         = leader.'gS'
    let maps["Add Cursor At Pos"][0]     = '<M-S-LeftMouse>'
    "let maps["Start Regex Search"][0]    = leader.'/'
    "let maps["Select All"][0]            = leader.'A'
   " let maps["Add Cursor Down"][0]       = '<A-Down>'
   " let maps["Add Cursor Up"][0]         = '<A-Up>'
  ...
  if g:VM_mouse_mappings
    ...
    "let maps["Mouse Cursor"][0]          = '<M-S-LeftMouse>'
    "let maps["Mouse Word"][0]            = '<M-S-LeftMouse>'
    "let maps["Mouse Column"][0]          = '<M-S-LeftMouse>'
  endif
